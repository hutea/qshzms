...
<div class="wrapall">
    ...
    <div class="hz-main"> 
    	...
    	<c:forEach items="${pageView.records}" var="entry" varStatus="s">  
                ...
                <div class="row hz-row-tag">
                    <div class="col-xs-8 col-sm-9 col-md-9"><c:if test="${fn:length(entry.tags)>0}"><i class="fa fa-tag" style="color:#368;"></i></c:if> 
                    	<c:forEach items="${entry.tags}" var="tag" varStatus="s">  
	                		<c:if test="${s.index%5==0}"> <a href="${pageContext.request.contextPath}/share/article/tag/${tag.id}" class="btn btn-primary btn-xs">${tag.name}</a></c:if>
	                		<c:if test="${s.index%5==1}"> <a href="${pageContext.request.contextPath}/share/article/tag/${tag.id}" class="btn btn-warning btn-xs">${tag.name}</a></c:if>
	                		<c:if test="${s.index%5==2}"> <a href="${pageContext.request.contextPath}/share/article/tag/${tag.id}" class="btn btn-info btn-xs">${tag.name}</a></c:if>
	                		<c:if test="${s.index%5==3}"> <a href="${pageContext.request.contextPath}/share/article/tag/${tag.id}" class="btn btn-danger btn-xs">${tag.name}</a></c:if>
	                		<c:if test="${s.index%5==4}"> <a href="${pageContext.request.contextPath}/share/article/tag/${tag.id}" class="btn btn-default btn-xs" >${tag.name}</a></c:if>
                		</c:forEach>
                    </div>
            ...
            <%@ include file="/WEB-INF/page/view/common/share_list_article_tag_fenye.jsp" %>
