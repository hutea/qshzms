...
<html lang="en">
    <head>
       ...
       <script type="text/javascript">
    	function del(bid){
			if (confirm('您确定要删除吗')) {
			  $.get("${pageContext.request.contextPath}/manage/blog/delete", 
			  {bid:bid},
			  function(data) {
		      	if(data=='success'){
		      		$("#tr_"+bid).css("display","none");
		       	}
			   });
			}
		}
    ...
    <div class="mainwrapper">
        ...
        <div class="mainpanel">
             ...
             <div class="contentpanel">    
             	 <div class="content" >
				      <div class="row">
				      	<form id="pageList" action="${pageContext.request.contextPath}/manage/blog/list" method="post">
                           ...
                           <thead>
                            ...
                            <tbody>
                                  <c:forEach items="${pageView.records}" var="entry" varStatus="s">  
	                           		 ...
	                           		 <td>${entry.id}</td> 
	                           		 <td>${entry.title}</td> 
	                           		 <td>${entry.blogType.name}</td> 
	                            	 <td><fmt:formatDate value="${entry.createDate}" pattern="yyyy-MM-dd hh:mm:ss "/></td> 
	                           		 <td>
	                           		 	<span id="lv_${entry.id}">${entry.lv}</span>
	                           		 </td> 
	                           		 <td>
	                           		 	<a href="javascript:del('${entry.id}')">删除</a>
	                           		 	<a href="${pageContext.request.contextPath}/manage/blog/edit/${entry.id}">修改</a>
	                           		 	<a href="${pageContext.request.contextPath}/blog/view/${entry.id}" target="_blank">查看</a>
	                           		 </td> 
	                           	  	</tr>
	                           	  </c:forEach>
                            </tbody>
