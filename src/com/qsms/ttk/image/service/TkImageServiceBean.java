...
public class TkImageServiceBean extends DAOSupport<TkImage> implements
	...
	public PageView<TkImage> queryByTag(int maxresult, int page,
		...
		Query query = em
				.createQuery(
						"select s from TkImage s left join s.share.tags t where s.share.visible=?1 and s.share.reported<=?2 and s.share.category=?3 "
								+ querySTR.toString()).setParameter(1, true)
		...
		Query queryCount = em
				.createQuery(
						"select count(s.id) from TkImage s left join s.share.tags t where s.share.visible=?1 and s.share.reported<=?2 and s.share.category=?3 "
